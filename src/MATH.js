...
export const arrAvg = (arr) => arrSum(arr) / arr.length;
...
export const pntOnLine = (p0, p1, t) => ({
  x: p0.x + (p1.x - p0.x) * t,
  y: p0.y + (p1.y - p0.y) * t,
});

export const decimals = (num, decimals) =>
  isNaN(num) ? null : (Math.round(num * 100) / 100).toFixed(decimals);
